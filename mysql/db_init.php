...
function CreaDB() {
    ...
    $query = 'CREATE TABLE if not exists PRINTS (id_PRINTS INT NOT NULL AUTO_INCREMENT PRIMARY KEY, titolo VARCHAR(100) NOT NULL, autore VARCHAR(100) NOT NULL,abstract VARCHAR(10000) NOT NULL, percorso VARCHAR(500) NOT NULL, timestamp TIMESTAMP, data_inserimento DATETIME NOT NULL, anno int NOT NULL, FOREIGN KEY (autore) REFERENCES AUTORI(uid))';
    $result = mysqli_query($db_connect, $query) or die('Query failed: ' . mysqli_error());
