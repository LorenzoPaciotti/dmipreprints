...
function CreaDB() {
    ...
    $query = 'CREATE TABLE if not exists AUTORI (uid VARCHAR(9) PRIMARY KEY, nome VARCHAR (50), cognome VARCHAR(50))';
    ...
    $query = 'CREATE TABLE if not exists PRINTS (id_PRINTS INT NOT NULL AUTO_INCREMENT PRIMARY KEY, titolo VARCHAR(100) NOT NULL, autore VARCHAR(100),abstract VARCHAR(10000), percorso VARCHAR(500),FOREIGN KEY (autore) REFERENCES AUTORI(uid))';
    $result = mysqli_query($db_connect, $query) or die('Query failed: ' . mysqli_error());
