...
function CreaDB() {
    ...
    $query = 'CREATE TABLE if not exists AUTORI (uid VARCHAR(9) PRIMARY KEY, nome VARCHAR (80) NOT NULL, moder TINYINT(1) default 0 NOT NULL)';
    $result = mysqli_query($db_connect, $query) or die('Query failed: ' . mysqli_error($db_connect). "query fatta: ".$query);
    ...
    $query = 'CREATE TABLE if not exists PRINTS (id_PRINTS INT NOT NULL AUTO_INCREMENT PRIMARY KEY, titolo VARCHAR(100) NOT NULL, autore VARCHAR(100) NOT NULL,abstract VARCHAR(10000) NOT NULL, nome_file VARCHAR(500) NOT NULL, timestamp TIMESTAMP, data_inserimento DATETIME NOT NULL, anno int NOT NULL, approvato TINYINT(1) default 0 NOT NULL, FOREIGN KEY (autore) REFERENCES AUTORI(uid))';
